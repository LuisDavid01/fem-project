...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	VALUES($1,$2,$3,$4,$5)
	RETURNING id`
	err = tx.QueryRow(query, workout.UserID, workout.Title, workout.Description, workout.Duration_minutes, workout.CaloriesBurned).Scan(&workout.ID)
	if err != nil {
		return nil, err
	}
...
func (pg *PostgresWorkoutStore) GetWorkoutById(id int64) (*Workout, error) {
	...
	WHERE id = $1
	`
	err := pg.db.QueryRow(query, id).Scan(&workout.ID, &workout.Title, &workout.Description, &workout.Duration_minutes, &workout.CaloriesBurned)
	if err == sql.ErrNoRows {
		return nil, err // No workout found
	}
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	WHERE id = $5
	`
	result, err := tx.Exec(query, workout.Title, workout.Description, workout.Duration_minutes, workout.CaloriesBurned, workout.ID)
	if err != nil {
		return err
	}
